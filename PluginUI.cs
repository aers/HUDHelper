    ...
    public unsafe class PluginUI
    {
        ...
        public void Draw()
        {
            ...
            if (ImGui.Begin("HUDHelper", ref visible, ImGuiWindowFlags.NoResize))
            {
                ...
                var X = (int) _p.hudLayoutManager.hudLayoutScreen->SelectedOverlayNode->AtkResNode.X;
                if (ImGui.InputInt("X", ref X, 1, 5, ImGuiInputTextFlags.EnterReturnsTrue))
                {
                    _p.hudLayoutManager.SetPositionGame((short)X, (short)_p.hudLayoutManager.hudLayoutScreen->SelectedOverlayNode->AtkResNode.Y);
                }
                var Y = (int)_p.hudLayoutManager.hudLayoutScreen->SelectedOverlayNode->AtkResNode.Y;
                if (ImGui.InputInt("Y", ref Y, 1, 5, ImGuiInputTextFlags.EnterReturnsTrue))
                {
                    _p.hudLayoutManager.SetPositionGame((short)_p.hudLayoutManager.hudLayoutScreen->SelectedOverlayNode->AtkResNode.X, (short)Y);
                }
                //float scale = addon->Scale;
                //if (ImGui.InputFloat("Scale", ref scale, 0, 0, null, ImGuiInputTextFlags.EnterReturnsTrue))
                //{
                //    _p.hudLayoutManager.SetScaleGame(scale);
                //}
